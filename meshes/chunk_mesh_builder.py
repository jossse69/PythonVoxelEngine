...
def get_chunk_index(world_voxel_pos):
    ...
    cz = wz // CHUNK_SIZE
    cy = wy // CHUNK_SIZE
    if not (0 <= cx < WORLD_WIDTH and 0 <= cz < WORLD_WIDTH and 0 <= cy < WORLD_HEIGHT):
        return -1
...
def is_void(local_voxel_pos, world_voxel_pos, world_voxels):
    chunk_id = get_chunk_index(world_voxel_pos)
    if chunk_id == -1:
        return False # out of world
    chunk_voxels = world_voxels[chunk_id]
...
@njit
def is_void_ignore_leaves(local_voxel_pos, world_voxel_pos, world_voxels):
    chunk_id = get_chunk_index(world_voxel_pos)
    if chunk_id == -1:
        return False # out of world
    chunk_voxels = world_voxels[chunk_id]
    ...
    for x in range(CHUNK_SIZE):
        for y in range(CHUNK_SIZE):
            for z in range(CHUNK_SIZE):
                ...
                # top face
                if is_void_ignore_leaves((x, y + 1, z), (wx, wy + 1, wz), world_voxels) or voxel_id == LEAVES:
                ...
                # right face
                if is_void_ignore_leaves((x + 1, y, z), (wx + 1, wy, wz), world_voxels) or voxel_id == LEAVES:
                ...
                # back face
                if is_void_ignore_leaves((x, y, z - 1), (wx, wy, wz - 1), world_voxels) or voxel_id == LEAVES:
